...
`detect_tag_changes.py` is a Python script that detects changes in normative tags extracted from AsciiDoc files. It compares two tag JSON files (typically generated by the `tags.rb` converter) and reports additions, deletions, and modifications.

## Usage

```bash
./detect_tag_changes.py [options] REFERENCE_TAGS.json CURRENT_TAGS.json
```
...
```bash
./detect_tag_changes.py build/reference-tags.json build/current-tags.json
```
...
```bash
./detect_tag_changes.py reference.json current.json --update-reference
```
...
```bash
./detect_tag_changes.py -u -v reference.json current.json
```
...
```bash
./detect_tag_changes.py --verbose reference.json current.json
```
...
```bash
./detect_tag_changes.py docs-v1.0-tags.json docs-v1.1-tags.json
```
...
```bash
if ./detect_tag_changes.py reference-tags.json current-tags.json; then
...
2. **Compare tags** between versions:
   ```bash
   ./detect_tag_changes.py document-v1.tags.json document-v2.tags.json
   ```

3. **Create normative rules** using `create_normative_rules.py` with reviewed tags
...
# Run tests
./detect_tag_changes.py build/test-v1.json build/test-v2.json
./detect_tag_changes.py --show-text build/test-v1.json build/test-v2.json
```
